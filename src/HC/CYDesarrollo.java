...
public class CYDesarrollo extends javax.swing.JPanel {
    ...
    public CYDesarrollo(EntityManagerFactory factory, PypAdmAsistCon pypAdmAsistCon, Integer usuario) {
        initComponents();
    ...
    private void save() {
        if (Estadofinal.equals("1")) {
            ...
            if (b == 0) {
                ...
                sav.newcrecimiento(idhc, pypAdmAsistCon.getIdControlPro().getIdProfesional().getId().toString(), fc);
                if (modulo_pyp.Modulo_PyP.d.listPacientes.año <= 5) {
                    sav.newcrecimientomeses(idhc, pypAdmAsistCon.getIdControlPro().getIdProfesional().getId().toString(), fc);
                } else {
                    sav.newcrecimientoaños(idhc, pypAdmAsistCon.getIdControlPro().getIdProfesional().getId().toString(), fc);
                }
                Object h[][] = Funciones.RetornarDatos(act.cargardatoshc(idhc));
                if (h[0][23].toString().equals("0")) {
